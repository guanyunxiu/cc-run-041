...
export class App {
  ...
  constructor() {
    ...
    this.grid = new GridView({
      ...
      onFill: (direction, range) => this.worker.fill(direction, range),
    });
  ...
  private buildToolbar(): HTMLDivElement {
    ...
    fillDown.textContent = '向下填充';
    fillDown.title = '以选区首行为源向下填充（Ctrl+D），相对引用随行平移，$ 锁定不动';
    fillDown.addEventListener('click', () => this.fillSelection('down'));
    bar.appendChild(fillDown);

    const fillRight = document.createElement('button');
    fillRight.textContent = '向右填充';
    fillRight.title = '以选区首列为源向右填充（Ctrl+R），相对引用随列平移，$ 锁定不动';
    fillRight.addEventListener('click', () => this.fillSelection('right'));
  ...
  private fillSelection(direction: 'down' | 'right'): void {
    for (const r of state.selection.ranges) {
      const degenerate = direction === 'down' ? r.bottom <= r.top : r.right <= r.left;
      if (degenerate) continue;
      this.worker.fill(direction, r);
    }
  }
